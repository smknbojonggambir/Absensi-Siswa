...
import { fetchDirectSpreadsheetData, fetchKelas, DEFAULT_KELAS } from '../services/api';
...
export const DashboardAdmin: React.FC<DashboardAdminProps> = ({ lang }) => {
    ...
    async (isBackground = false, signal?: AbortSignal) => {
      ...
      try {
        const [allData, classes] = await Promise.all([
          fetchDirectSpreadsheetData(signal),
          fetchKelas(signal),
        ]);
        ...
        if (Array.isArray(classes)) {
          setKelasList(Array.from(new Set([...DEFAULT_KELAS, ...(classes || [])])).sort());
        }
        setLastRefreshed(
          new Date().toLocaleTimeString(isEn ? 'en-US' : 'id-ID', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
          })
  ...
  // Initial fetch with AbortController for clean disposal
  useEffect(() => {
  ...
  // Live Mode 30-second polling effect with AbortController disposal
  useEffect(() => {
    if (!isLiveMode) {
      setCountdown(30);
      return;
    ...
    const pollInterval = setInterval(async () => {
      // Cleanly abort any prior in-flight poll request before starting a new one
      if (pollController) {
        pollController.abort();
      }
      pollController = new AbortController();
      activeControllerRef.current = pollController;

      await fetchDashboardData(true, pollController.signal);
      setCountdown(30);
    }, 30000);

    // Clean disposal on Live Mode toggle off or component unmount
    return () => {
    ...
    <div style={{ padding: '8px 0' }} className="no-print">
      ...
      >
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '6px' }}>
            <span>{isEn ? 'Real-time attendance metrics & graphical analysis' : 'Analisis visual & ringkasan kehadiran siswa SMKN Bojonggambir'}</span>
            {lastRefreshed && (
            ...
            {isLiveMode && (
              ...
              >
                ...
                />
                {isPolling
                  ? isEn ? 'Live: Polling data...' : 'Live: Memeriksa data baru...'
                  : isEn ? `Live: Next poll in ${countdown}s` : `Live: Sinkronisasi dalam ${countdown}d`}
              </span>
            )}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px' }}>
          ...
          {/* 'Live Mode' Toggle Button (polls every 30 seconds with clean AbortController disposal) */}
          <button
            type="button"
            ...
            title={
              isLiveMode
                ? isEn
                  ? 'Turn off 30s Live Mode'
                  : 'Nonaktifkan Mode Live (polling 30 detik)'
                : isEn
                ? 'Turn on 30s Live Mode'
                : 'Aktifkan Mode Live (polling 30 detik)'
            }
            style={{
              marginTop: 0,
              padding: '8px 14px',
              ...
              backgroundColor: isLiveMode ? '#059669' : '#ffffff',
              color: isLiveMode ? '#ffffff' : '#334155',
              border: isLiveMode ? '1.5px solid #047857' : '1.5px solid #cbd5e1',
              boxShadow: isLiveMode ? '0 2px 8px rgba(5, 150, 105, 0.25)' : '0 1px 2px rgba(0,0,0,0.05)',
            }}
          >
            <span
              style={{
                ...
                height: '8px',
                borderRadius: '50%',
                backgroundColor: isLiveMode ? '#34d399' : '#94a3b8',
                boxShadow: isLiveMode ? '0 0 8px #34d399' : 'none',
              }}
            />
            <span>
              {isLiveMode ? (
                <>
                  <span>{isEn ? 'Live (30s)' : 'Live Aktif'}</span>
                  <span
                    style={{
                      fontSize: '11px',
                      opacity: 0.95,
                  ...
                  >
                    {isPolling ? (isEn ? 'Syncing...' : 'Sinkron...') : `${countdown}s`}
                  </span>
                </>
              ) : (
                <span>{isEn ? '⚡ Live Mode' : '⚡ Mode Live'}</span>
              )}
            </span>
          </button>
          ...
          >
            <span
              style={{
                display: 'inline-block',
                transform: loading || isPolling ? 'rotate(360deg)' : 'none',
